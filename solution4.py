...
def pips_solver(puzzle:dict[str,list]) -> list[tuple[int,int,int,int]]:
    ...
    def remaining_min_sum() -> int:
        num_non_sum_squares = 0
        to_return = 0
        for region in regions:
            if region['type'] == 'sum':
                curr_min_sum = region['target']
            elif region['type'] == 'greater':
                curr_min_sum = region['target']+1
            else:
                num_non_sum_squares += sum(
                    1 for spot in region['indices']
                    if spot in remaining_spots
                )
                continue
            for spot in region['indices']:
                curr_min_sum -= spots_to_pips.get(spot,0)
            to_return += max(0,curr_min_sum)
        sorted_pip_counts = sorted(
            p for idx in remaining_domino_idxs
            for p in dominoes[idx]
        )
        to_return += sum(sorted_pip_counts[:num_non_sum_squares])
        return to_return
    
    def remaining_domino_sum() -> int:
        return sum(
            sum(dominoes[idx])
    ...
    def helper(even_spot_idx:int) -> Optional[list[tuple[int,int,int,int]]]:
        ...
        if remaining_min_sum() > remaining_domino_sum():
            return
